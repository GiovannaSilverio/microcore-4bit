module decodificador(
	input [3:0] data,
	output reg [6:0] disp

);


	always @(*) begin
		case(data)
			4'b0000: disp = 7'b1000000; //0
			4'b0001: disp = 7'b1111001;  //1
			4'b0010: disp = 7'b0100100 ; //2
			4'b0011: disp = 7'b0110000;//3
			4'b0100: disp = 7'b0011001 ;//4
			4'b0101: disp = 7'b0010010; //5
			4'b0110: disp = 7'b0000010; //6
			4'b0111: disp = 7'b1111000; //7
			4'b1000: disp = 7'b0000000; //8
			4'b1001: disp = 7'b0010000; //9
			4'b1010: disp = 7'b0001000; //10 ou A
			4'b1011: disp = 7'b0000011 ;//11 ou B
			4'b1100: disp = 7'b1000110; //12 ou C
			4'b1101: disp = 7'b0100001; //13 ou D
			4'b1110: disp = 7'b0000110; //14 ou E
			4'b1111: disp = 7'b0001110 ;//15 ou F
		endcase
	end
			


endmodule